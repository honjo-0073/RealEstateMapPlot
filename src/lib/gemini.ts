    ...
    const result = await model.generateContent([
      {
        text: `不動産の物件概要PDFから仕入マップ登録用JSONを抽出してください。\n` +
          `必ずJSONのみを返してください。キーは name, business_item_registrant_name, asset_type, address, price_amount_yen, price_raw_text, land_area_sqm, floor_area_sqm, zoning, coverage_ratio_raw, road_access, transaction_type, latitude, longitude, notes, visibility です。\n` +
